...
if grep -q "^#net.ipv4.ip_forward=1" /etc/sysctl.conf; then sed -i 's/^#net.ipv4.ip_forward=1/net.ipv4.ip_forward=1/' /etc/sysctl.conf; elif ! grep -q "^net.ipv4.ip_forward=1" /etc/sysctl.conf; then echo "net.ipv4.ip_forward=1" >> /etc/sysctl.conf; fi &&
sysctl -p && 
mkdir /etc/ocserv/ssl/ &&
cd /etc/ocserv/ssl/ &&
mkdir -p /etc/ocserv/ssl/ && printf "organization = \"Org\"\ncn = \"CA\"\nserial = 001\nexpiration_days = -1\nca\nsigning_key\ncert_signing_key\ncrl_signing_key\n" > /etc/ocserv/ssl/ca.tmpl &&
mkdir -p /etc/ocserv/ssl/ && printf "organization = \"Server Org\"\ncn = \"server\"\nserial = 002\nexpiration_days = 3650\nca\nsigning_key\nencryption_key\ntls_www_server\nip_address = \"5.101.44.90\"\n" > /etc/ocserv/ssl/server.tmpl  &&
